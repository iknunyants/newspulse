...
- [~] **B8. Keyword Refresh** — ~~Dropped.~~ Regenerating keywords with the same model and same topic text produces essentially identical output (model has a fixed training cutoff, no access to recent events). This would waste LLM API calls with no benefit.
